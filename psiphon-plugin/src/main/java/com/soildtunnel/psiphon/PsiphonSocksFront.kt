...
class PsiphonSocksFront {
    ...
    private var psiphonPort: Int = PSIPHON_LOCAL_PORT

    fun start(frontPort: Int, psiphonPort: Int) {
        this.psiphonPort = psiphonPort
        server = ServerSocket().apply {
            reuseAddress = true
            bind(InetSocketAddress(PSIPHON_SOCKS_HOST, frontPort))
        }
        running.set(true)
    ...
    private fun handle(client: Socket) {
        ...
        try {
            ...
            upstream = Socket()
            upstream.connect(InetSocketAddress(PSIPHON_SOCKS_HOST, psiphonPort), 10000)
    ...
    private fun serveUdpAssociate(client: Socket, input: InputStream, out: OutputStream) {
        val relay = java.net.DatagramSocket(java.net.InetSocketAddress(PSIPHON_SOCKS_HOST, 0))
    ...
    private fun psiphonDnsQuery(query: ByteArray): ByteArray? = runCatching {
        val s = Socket()
        s.connect(InetSocketAddress(PSIPHON_SOCKS_HOST, psiphonPort), 10000)
        s.soTimeout = 15000
