...
pub async fn run_with(args: Vec<String>) -> Result<()> {
    ...
    let default_filter = format!("info,soildtunnel={level}");
    ...
    log::info!("SoildTunnel v{}", env!("CARGO_PKG_VERSION"));
    sysprofile::log_summary();

    install_netstack_panic_guard();

    let listen: SocketAddr = std::env::var("SOILDTUNNEL_SOCKS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or_else(|| "127.0.0.1:1819".parse().unwrap());

    let base_config = std::env::var("SOILDTUNNEL_CONFIG").unwrap_or_else(|_| DEFAULT_CONFIG.to_string());

    let protocol = if std::env::var("SOILDTUNNEL_PEER").is_ok() || std::env::var("SOILDTUNNEL_WG_PEER").is_ok() {
        match std::env::var("SOILDTUNNEL_PROTOCOL") {
...
fn noize_config() -> noize::NoizeConfig {
    let profile = std::env::var("SOILDTUNNEL_NOIZE").unwrap_or_else(|_| "firewall".to_string());
    log::info!("[+] obfuscation profile: {profile}");
    noize::from_profile(&profile)
}

fn soildnoize_config() -> soildnoize::SoildNoizeConfig {
    let profile = std::env::var("SOILDTUNNEL_NOIZE").unwrap_or_else(|_| "balanced".to_string());
    log::info!("[+] soildnoize profile: {profile}");
    soildnoize::from_profile(&profile)
}
...
async fn enrol_zero_trust(base: &str) {
    ...
    std::env::set_var("SOILDTUNNEL_TEAM", &team);
    ...
    let needs_method = match zerotrust::TeamSettings::from_env() {
        ...
        None => {
            std::env::remove_var("SOILDTUNNEL_TEAM");
            return;
        }
    };
    ...
    if needs_method {
        ...
        std::env::set_var("SOILDTUNNEL_ACCESS_EMAIL", &email);
    }
    ...
    match zerotrust::resolve_token(&settings).await {
        ...
        Err(error) => {
            ...
            std::env::remove_var("SOILDTUNNEL_TEAM");
            std::env::remove_var("SOILDTUNNEL_ACCESS_EMAIL");
        }
    }
}
...
async fn adopt_team_profile(identity: account::Identity) -> account::Identity {
    ...
    if !identity.gateway_proxy.is_empty() {
        if std::env::var("SOILDTUNNEL_GATEWAY").is_ok() {
    ...
    if !identity.assigned_endpoint.is_empty() && std::env::var("SOILDTUNNEL_PEER").is_err() {
        let port = if std::env::var("SOILDTUNNEL_PROTOCOL")
        ...
        if peer.parse::<SocketAddr>().is_ok() {
            ...
            std::env::set_var("SOILDTUNNEL_TEAM_ENDPOINT", &peer);
        }
    }
...
fn warp_config_path(base: &str) -> String {
    if let Ok(p) = std::env::var("SOILDTUNNEL_WG_CONFIG") {
        return p;
    }
    match team_scope() {
...
fn masque_config_path(base: &str) -> String {
    if let Ok(p) = std::env::var("SOILDTUNNEL_MASQUE_CONFIG") {
        return p;
    }
    match team_scope() {
        Some(team) => derive_sibling_path(base, &format!("team-{team}")),
...
fn keep_saved_identity() -> bool {
    !matches!(
        std::env::var("SOILDTUNNEL_REPROVISION").as_deref(),
    ...
    if let Some(identity) = config::load(config_path)? {
        ...
        let refused = if identity.has_masque_credentials() {
            ...
            match account::ensure_masque_enrolled(&identity).await {
                ...
                Err(SoildTunnelError::IdentityRefused(reason)) => {
...
async fn select_peer(identity: &account::Identity, protocol: Protocol) -> Result<SocketAddr> {
    let force_peer = match protocol {
        Protocol::Masque => std::env::var("SOILDTUNNEL_PEER").ok(),
        Protocol::WireGuard | Protocol::WarpInWarp => std::env::var("SOILDTUNNEL_WG_PEER")
            .ok()
            .or_else(|| std::env::var("SOILDTUNNEL_PEER").ok()),
    };
    
    if let Some(p) = force_peer {
        let peer: SocketAddr = p
            .parse()
            .map_err(|_| SoildTunnelError::Other(format!("bad peer address {p}")))?;
...
async fn select_wg_peers(
    ...
    let probe = wg_prober::WgProbe {
        ...
        local_ipv4: identity
            .ipv4
            .parse()
            .map_err(|_| SoildTunnelError::Other("invalid ipv4".into()))?,
        soildnoize: soildnoize_config(),
...
async fn resolve_ech() -> Option<Vec<u8>> {
    match std::env::var("SOILDTUNNEL_ECH") {
        ...
        Ok(b64) if !b64.is_empty() => match tls::decode_ech_config_list(&b64) {
            Ok(v) => {
                log::info!("[+] using ECHConfigList from SOILDTUNNEL_ECH");
                Some(v)
            }
            Err(e) => {
                log::warn!("[-] bad SOILDTUNNEL_ECH: {e}; continuing without ECH");
                None
            }
        },
        _ => {
...
fn masque_reconnect_delay() -> std::time::Duration {
    let secs = std::env::var("SOILDTUNNEL_MASQUE_RECONNECT_SECS")
        .ok()
...
fn masque_startup_timeout() -> std::time::Duration {
    let secs = std::env::var("SOILDTUNNEL_MASQUE_STARTUP_SECS")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .filter(|&v| v > 0)
        .unwrap_or(30);
...
async fn want_quick_reconnect(cached: &lastconn::LastConnection) -> bool {
    match std::env::var("SOILDTUNNEL_QUICK_RECONNECT").as_deref() {
...
async fn run_masque(
    ...
    if forced.is_none() {
        if let Some(assigned) = std::env::var("SOILDTUNNEL_TEAM_ENDPOINT")
            .ok()
...
async fn run_masque_tunnel(
    ...
    match tokio::time::timeout(startup_timeout, ready_rx).await {
        ...
        Ok(Err(_)) => {
            ...
            return Err(SoildTunnelError::Other(msg));
        }
        Err(_) => {
            tunnel_task.abort();
            let _ = tunnel_task.await;
            return Err(SoildTunnelError::Other(format!(
...
fn wg_keepalive_secs() -> u16 {
    std::env::var("SOILDTUNNEL_WG_KEEPALIVE")
        .ok()
...
fn wg_profile_candidates() -> Vec<(String, soildnoize::SoildNoizeConfig)> {
    let primary = std::env::var("SOILDTUNNEL_NOIZE").unwrap_or_else(|_| "balanced".to_string());
    log::info!("[+] soildnoize primary profile: {primary}");

    let mut names = vec![primary.clone()];
    if std::env::var("SOILDTUNNEL_WG_NO_PROFILE_RETRY").is_err() {
    ...
    names
        .into_iter()
        .map(|n| {
            let cfg = soildnoize::from_profile(&n);
            (n, cfg)
        })
        .collect()
}
...
async fn hunt_wg_peer_with_profile(
    ...
    mode_str: &str,
    ip: prober::IpScan,
    profile: soildnoize::SoildNoizeConfig,
...
fn wg_reconnect_delay() -> std::time::Duration {
    let secs = std::env::var("SOILDTUNNEL_WG_RECONNECT_SECS")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .unwrap_or(2);
    std::time::Duration::from_secs(secs)
...
fn wg_endpoint_cooldown() -> std::time::Duration {
    let secs = std::env::var("SOILDTUNNEL_WG_ENDPOINT_COOLDOWN_SECS")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .filter(|&v| v > 0)
        .unwrap_or(300);
...
async fn hunt_wg_peer(
    identity: &account::Identity,
    candidates: &[(String, soildnoize::SoildNoizeConfig)],
    mode_str: &str,
    ip: prober::IpScan,
    excluded: &HashSet<SocketAddr>,
) -> Result<(SocketAddr, soildnoize::SoildNoizeConfig, String)> {
    let multi = candidates.len() > 1;
    for (name, profile) in candidates {
        log::info!(
            "[*] hunting for a working WireGuard endpoint (handshake + data-plane verification, soildnoize='{name}')"
        );
        match hunt_wg_peer_with_profile(identity, mode_str, ip, profile.clone(), excluded).await {
            Ok(peer) => {
                log::info!("[+] selected WireGuard endpoint {peer} using soildnoize profile '{name}'");
    ...
    Err(SoildTunnelError::NoCleanEndpoint)
}
...
async fn run_wireguard(identity: account::Identity, listen: SocketAddr, lastconn_path: String) -> Result<()> {
    ...
    let forced = std::env::var("SOILDTUNNEL_WG_PEER")
        .ok()
        .or_else(|| std::env::var("SOILDTUNNEL_PEER").ok());
    ...
    let mut quick: Option<(SocketAddr, soildnoize::SoildNoizeConfig, String)> = None;
    ...
    if forced.is_none() && quick.is_none() {
        if let Some(cached) = lastconn::load(&lastconn_path) {
            if let Ok(peer) = cached.peer.parse::<SocketAddr>() {
                if want_quick_reconnect(&cached).await {
                    let profile = soildnoize::from_profile(&cached.profile);
    ...
    let mut last_good: Option<(SocketAddr, soildnoize::SoildNoizeConfig, String)> = None;
            ...
            match retried {
                ...
                None => {
                    if let Some(ref p) = forced {
                        ...
                        let mut chosen = None;
                        for (name, profile) in &candidates {
                            log::info!("[*] testing forced peer {peer} with soildnoize profile '{name}'");
                            match wireguard::verify_endpoint(
                                peer,
                                private_key,
                                peer_public,
                        ...
                        match chosen {
                            Some(v) => v,
                            None => return Err(SoildTunnelError::NoCleanEndpoint),
                        }
                    } else {
...
fn wg_tunnel_validate_timeout() -> std::time::Duration {
    let secs = std::env::var("SOILDTUNNEL_WG_VALIDATE_SECS")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .filter(|&v| v > 0)
        .unwrap_or(10);
...
async fn run_wireguard_tunnel(
    identity: account::Identity,
    peer: SocketAddr,
    soildnoize: soildnoize::SoildNoizeConfig,
    listen: SocketAddr,
) -> Result<()> {
    let private_key = identity.private_key_bytes()?;
    let peer_public = identity.peer_public_key_bytes()?;
    ...
    .await
    .map_err(|e| SoildTunnelError::Other(format!("tunnel failed validation: {e}")))?;
    ...
    let tunnel = wireguard::WgTunnel::from_established(session, std::sync::Arc::new(soildnoize), inbound_tx, ipv4);
    ...
    match tunnel_result {
        Ok(()) => Ok(()),
        Err(e) => Err(SoildTunnelError::Other(format!("wireguard tunnel exited: {e}"))),
    }
}
...
fn http_proxy_listen() -> Option<SocketAddr> {
    let raw = std::env::var("SOILDTUNNEL_HTTP_PROXY").ok()?;
...
async fn establish_wg(
    ...
    .await
    .map_err(|e| SoildTunnelError::Other(format!("[{label}] tunnel failed validation: {e}")))?;
    ...
    let exit = tokio::spawn(async move {
        match tunnel.run(outbound_rx).await {
            ...
            Err(e) => {
                ...
                Err(SoildTunnelError::Other(format!("[{label}] {e}")))
            }
        }
    });
...
fn join_outcome(
    ...
    match result {
        Ok(Ok(())) => Err(SoildTunnelError::Other(format!("{what} stopped"))),
        Ok(Err(e)) => Err(e),
        Err(e) if e.is_cancelled() => Err(SoildTunnelError::Other(format!("{what} was cancelled"))),
        Err(e) => Err(SoildTunnelError::Other(format!("{what} panicked: {e}"))),
    }
}
...
async fn select_scan_mode() -> prober::ScanMode {
    if let Ok(v) = std::env::var("SOILDTUNNEL_SCAN") {
...
async fn select_protocol(base: &str) -> Protocol {
    if let Ok(v) = std::env::var("SOILDTUNNEL_PROTOCOL") {
...
async fn select_ip_version() -> prober::IpScan {
    if let Ok(v) = std::env::var("SOILDTUNNEL_IP") {
