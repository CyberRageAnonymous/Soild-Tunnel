...
- **WARP×2 country pin** — when a WARP×2 edge is pinned (e.g. Germany), the engine now forces a fresh scan of that range and clears stale quick-reconnect caches; MASQUE and WireGuard no longer show a server picker because they dial the same anycast edge.
- **WARP×2 MTU** — the Advanced MTU setting now reaches the double-tunnel engine (outer/inner tunnel MTUs follow it, clamped to a safe 1280–1500). Raising the MTU on a clean network gives WARP×2 bigger TCP segments and room for full-size QUIC packets; the stock default is unchanged and stays safe on filtered networks.
