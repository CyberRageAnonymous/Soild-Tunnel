...
import com.soildtunnel.app.SoildTunnelApp
import com.soildtunnel.app.MainActivity
import com.soildtunnel.app.R
import com.soildtunnel.app.core.SoildTunnelController
import com.soildtunnel.app.core.SoildTunnelProcess
import com.soildtunnel.app.core.Diagnostics
...
import com.soildtunnel.app.widget.SoildTunnelWidgetProvider
import java.io.File
...
class SoildTunnelVpnService : VpnService() {
    ...
    private fun startTunnel(profile: ConnectionProfile) {
        ...
        runJob = scope.launch {
            ...
            try {
                ...
                SoildTunnelController.setState(
    ...
    private suspend fun connectFlow(profile: ConnectionProfile) {
        ...
        // Desktop-parity info row : publish the protocol that actually
    ...
    private suspend fun connectAttempt(
        ...
        DiagnosticsLog.i(TAG, "Launching engine (libsoildtunnel-core.so)…")
        ...
        if (profile.proxyMode) {
            ...
            // Ports are FIXED (well-known standard values) — the same values are
    ...
    private suspend fun superviseEngine(profile: ConnectionProfile) {
        ...
        while (currentScopeActive()) {
            ...
            if (attempt >= maxRetries(profile)) {
                // KILL SWITCH : instead of tearing the VPN down and
    ...
    private fun establishTun(profile: ConnectionProfile) {
        ...
        // KILL SWITCH : a blocking interface never falls back to
    ...
    /**
     * Writes the hev-socks5-tunnel config in the proven production shape.
     *
    ...
    private fun stopEverything() {
        ...
        stopJob = scope.launch(Dispatchers.IO) {
            ...
            Diagnostics.resetChecks()
            EngineMeta.reset()
            SoildTunnelController.setState(ConnectionState.Idle)
            SoildTunnelTileService.requestUpdate(this@SoildTunnelVpnService)
    ...
    /** Max automatic engine restarts (Smart Reconnect, ). */
    ...
    private fun enterLockdown(profile: ConnectionProfile) {
        ...
        stopJob = scope.launch(Dispatchers.IO) {
            ...
            Diagnostics.resetChecks()
            EngineMeta.reset()
            SoildTunnelController.setState(ConnectionState.Error(getString(R.string.state_killswitch)))
    ...
    private fun ensureLockdownTun(profile: ConnectionProfile) {
        ...
        tun = null
        val builder = Builder()
            .setSession("SoildTunnel KillSwitch")
    ...
    private fun buildNotification(text: String): android.app.Notification {
        ...
        val disconnectIntent = PendingIntent.getService(
            this,
            1,
            Intent(this, SoildTunnelVpnService::class.java).apply { action = ACTION_DISCONNECT },
            PendingIntent.FLAG_IMMUTABLE or PendingIntent.FLAG_UPDATE_CURRENT,
        )
        return NotificationCompat.Builder(this, SoildTunnelApp.CHANNEL_ID)
    ...
    private fun updateNotification(text: String) {
        ...
        SoildTunnelTileService.requestUpdate(this)
        // Keep the home-screen widget (feature merge) in sync too.
        // Cheap: returns immediately when no widget is placed.
        SoildTunnelWidgetProvider.updateAllWidgets(this)
    }

    companion object {
        const val ACTION_CONNECT = "com.soildtunnel.app.CONNECT"
        const val ACTION_DISCONNECT = "com.soildtunnel.app.DISCONNECT"
        ...
        /**
         * Consecutive failed checks before the engine is restarted (
