package com.soildtunnel.app.model
...
data class ConnectionProfile(
     ...
     * in-app proxy bridge (see [com.soildtunnel.app.core.ShareBridge]).
     ...
     * scans exactly these ranges (see SOILDTUNNEL_SCAN_CIDRS in prober.rs), minus
    ...
    /** TUN interface MTU. 1500 = standard Ethernet frame size; throughput-optimal. */
     ...
     * only from the address (`SOILDTUNNEL_ROUTE_SNIFF`).
     *
     ...
     * one (`SOILDTUNNEL_REPROVISION`). Off means the tunnel still
    ...
    /** Command-line arguments passed to the `soildtunnel` engine binary. */
    ...
    fun toEnv(): Map<String, String> = buildMap {
        ...
        if (endpointMode == EndpointMode.MANUAL_RANGE && userRange.isNotBlank()) {
            ...
            put("SOILDTUNNEL_MASQUE_CIDRS", userRange)
            put("SOILDTUNNEL_WG_CIDRS", userRange)
        }
            ...
            when (teamAuth) {
                TeamAuth.SERVICE_TOKEN -> {
                    accessClientId.trim().takeIf { it.isNotEmpty() }
                        ?.let { put("SOILDTUNNEL_ACCESS_CLIENT_ID", it) }
                    accessClientSecret.trim().takeIf { it.isNotEmpty() }
                        ?.let { put("SOILDTUNNEL_ACCESS_CLIENT_SECRET", it) }
                }
                TeamAuth.EMAIL ->
                    accessEmail.trim().takeIf { it.isNotEmpty() }
                        ?.let { put("SOILDTUNNEL_ACCESS_EMAIL", it) }
                TeamAuth.TOKEN ->
                    accessToken.trim().takeIf { it.isNotEmpty() }
                        ?.let { put("SOILDTUNNEL_ACCESS_TOKEN", it) }
                TeamAuth.OFF -> Unit
            }
        }
        ...
        if (reconnectSecs > 0) {
            put("SOILDTUNNEL_MASQUE_RECONNECT_SECS", reconnectSecs.coerceIn(1, 600).toString())
            put("SOILDTUNNEL_WG_RECONNECT_SECS", reconnectSecs.coerceIn(1, 600).toString())
        }
        ...
        if (!routeSniff) put("SOILDTUNNEL_ROUTE_SNIFF", "0")
        if (routeSniff && routeSniffMs > 0) {
            put("SOILDTUNNEL_ROUTE_SNIFF_MS", routeSniffMs.coerceIn(50, 5_000).toString())
        }
        if (!autoReprovision) put("SOILDTUNNEL_REPROVISION", "0")
    ...
    /**
     * Validated upstream proxy URL for `SOILDTUNNEL_UPSTREAM`. Accepts an explicit
    ...
    fun connectTimeoutMs(): Long {
        // Tor has to build circuits first, which takes a while on a first
        // connect, not seconds like WARP.
    ...
    companion object {
        /**
         * Default TUN MTU = 1500 (standard Ethernet frame size). Larger TUN
         * frames mean fewer read/write round-trips per byte for the tunnel
         * readers, which measurably raises throughput; the underlay TCP
         * connection segments payloads itself, so no path-MTU risk like the
         * old 8500 experiment had.
         */
