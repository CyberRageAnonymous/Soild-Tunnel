...
android {
    ...
    defaultConfig {
        ...
        val coreVersion = rootProject.file("native/engine/CORE_VERSION")
            .takeIf { it.exists() }?.readText()?.trim().orEmpty().ifBlank { "unknown" }
        buildConfigField("String", "CORE_VERSION", "\"$coreVersion\"")
    }

    // Both native cores (libhev-socks5-tunnel.so + libsoildtunnel-core.so) are
    // prebuilt by scripts/build-natives.sh into src/main/jniLibs, so there is NO
...
dependencies {
    ...
    // Tor network: the daemon binary (libtor.so per ABI) plus the pluggable
    // transports (obfs4/snowflake/webtunnel/meek) for bridged entry. Both
